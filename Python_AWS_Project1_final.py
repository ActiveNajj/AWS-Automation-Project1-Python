...
session = boto3.session.Session(profile_name="your_Admin_Profile")  # Use a profile_name on your machine which has admin privileges
...
instance_ids = []
for subnet_id, user_data_script in zip(public_subnet_ids, user_data_scripts):
...
response = autoscaling_client.describe_auto_scaling_groups(AutoScalingGroupNames=['AutoScalingGroup', ])
print(
    f"The Auto Scaling Group has been created successfully , its arn is {response['AutoScalingGroups'][0]['AutoScalingGroupARN']}")

''' 25 - create the RDS DataBase in private subnets and its security group'''
...
# D- Launch The Multi-AZ RDS database in private subnets
response = rds_client.create_db_instance(
    ...
    MasterUserPassword='NewdbPassword',
    DBSubnetGroupName='myrdsdbsubnetgroup',
...
waiter.wait(DBInstanceIdentifier='NewDBInstance')
print("RDS Instance is up and available now")
rds = rds_client.describe_db_instances(DBInstanceIdentifier='NewDBInstance')
